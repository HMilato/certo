...
//         Retorna os items seleccionados na arvore
        ...
        if (lb.getSelectedItems().isEmpty()) {
            Clients.showNotification("Por favor seleccione pelo menos uma acção!", "warning", null, null, 3000);
            return;
        }
...
//        }
        if (mDialogAddGroup.getTitle().compareTo("Adicionar Grupo") == 0) {
            ...
            li2.addAll(li);
            Messagebox.show("Modificar?", "Confirma��o", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION,
                    new EventListener() {
                        @Override
    ...
    private void addGroupConstraint() {
        txtGrupo.setConstraint(" no Empty: Insira o Nome do grupo!");
        txtDesc.setConstraint(" no Empty: Insira a Descri��o do grupo!");
    }
