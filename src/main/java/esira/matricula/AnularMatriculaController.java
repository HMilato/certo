...
public class AnularMatriculaController extends GenericForwardComposer {
    ...
    public void onRegAnulacao() throws IOException {
        ...
        if (validar()) {
            erroValidacao.setValue("Alguns Campos est�o vazios. Preencha Todos os campos");
        } else {
            if (validarAnoVolta()) {
                erroValidacao.setValue("O ano de Volta est� fora dos par�metros");
            } else {
                Messagebox.show("Pretende anular esta Matricula?", "Aten��o", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION,
                        new EventListener() {
                            @Override
    ...
    public void anular(Matriculaanulada mat, Matricula ma, List<Inscricaodisciplina> lid) throws IOException {
        ...
        if (idlitem.getValue() != -1) {
            ...
            Clients.showNotification("Pedido de Anula��o de Matricula Enviado com sucesso", null, null, null, 2000);
            lba.setValue("O Seu pedido para anular a Matricula foi enviado. \n Por favor aguarde a confirma��o...");
